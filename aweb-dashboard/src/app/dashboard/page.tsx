...
import { 
  FaClock, FaUserInjured, FaHandsHelping, 
  FaPhoneAlt, FaCheckCircle, FaCalendarAlt, FaExclamationCircle, FaTint 
} from 'react-icons/fa';
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-8 pt-8">
          ...
          <div className="flex justify-between items-center mb-8">
            ...
            <div className="hidden sm:flex gap-4 text-[10px] font-black uppercase tracking-widest">
              <div className="flex items-center gap-2 text-slate-400 italic">
                <span className="w-2.5 h-2.5 rounded-full bg-red-500"></span> Booked Date
              </div>
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-12">
          ...
          <div className="bg-white p-6 rounded-3xl shadow-sm border border-slate-200">
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-xl font-extrabold text-slate-800">Recipients Today</h2>
              <span className="text-[10px] bg-blue-100 text-blue-700 px-3 py-1 rounded-full uppercase font-black">Priority</span>
            </div>
            <div className="space-y-4">
              {todayPatients.length === 0 ? <EmptyState text="No patients registered today." /> : 
                todayPatients.map(p => (
                  <PersonActionCard 
                    key={p.id} name={p.name} group={p.blood_group} phone={p.phone} 
                    status={p.status || ''} 
                    onAction={p.status === 'Donated' ? () => handleCompletion(p.id) : undefined} 
                    btnLabel="Mark Completed" colorTheme="blue" 
                  />
                ))
              }
            </div>
          ...
          <div className="bg-white p-6 rounded-3xl shadow-sm border border-slate-200">
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-xl font-extrabold text-slate-800">Arriving Donors</h2>
              <span className="text-[10px] bg-red-100 text-red-700 px-3 py-1 rounded-full uppercase font-black">Active</span>
            </div>
            <div className="space-y-4">
              {todayDonors.length === 0 ? <EmptyState text="No donors arriving today." /> : 
                todayDonors.map(d => (
                  <PersonActionCard 
                    key={d.id} name={d.name} group={d.blood_group} phone={d.phone} 
                    status={d.status || ''} 
                    onAction={d.status === 'Accepted' ? () => handleDonation(d.id) : undefined} 
                    btnLabel="Mark Donated" colorTheme="red" 
                  />
                ))
              }
            </div>
        ...
        <div className="bg-white p-8 rounded-3xl shadow-sm border border-slate-200">
          <h2 className="text-xl font-extrabold text-slate-800 mb-8 flex items-center gap-3 uppercase tracking-tighter">
            <FaClock className="text-emerald-500" /> Upcoming Operations Pipeline
          </h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {upcomingAppts.map(appt => (
              <div key={appt.id} className="p-5 border border-slate-100 rounded-2xl bg-[#FBFCFD] flex flex-col justify-between group hover:border-red-200 transition-all cursor-default">
                ...
                <div className="flex items-center gap-2 text-xs text-slate-500">
                  ...
                  <span>{new Date(appt.date).toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: 'numeric'})}</span>
                </div>
              </div>
            ))}
          </div>
...
function PersonActionCard({ name, group, phone, status, btnLabel, colorTheme, onAction }: PersonActionCardProps) {
  const isDone = status === 'Completed' || status === 'Donated';
  const themeClasses = colorTheme === 'red' 
    ? 'text-red-600 bg-red-50 border-red-100' 
    : 'text-blue-600 bg-blue-50 border-blue-100';

  return (
    <div className={`p-5 rounded-2xl border transition-all ${isDone ? 'bg-slate-50 opacity-60 border-slate-200 shadow-none' : 'bg-white shadow-sm border-slate-100 hover:border-slate-300'}`}>
      <div className="flex justify-between items-start">
        <div className="flex-1">
          <h4 className="font-bold text-slate-900 leading-tight text-lg">{name}</h4>
          <div className="flex gap-3 mt-3">
            <span className={`text-[9px] font-black px-2.5 py-1 border rounded-lg ${themeClasses} flex items-center gap-1`}>
              <FaTint size={8} /> {group}
            </span>
            <a href={`tel:${phone}`} className="text-[11px] font-bold text-slate-400 flex items-center gap-1.5 hover:text-slate-700 transition-colors">
              <FaPhoneAlt size={10} className="text-slate-300" /> {phone}
            </a>
          </div>
        </div>
        <div className={`px-3 py-1 rounded-full text-[9px] font-black uppercase tracking-widest border ${isDone ? 'bg-emerald-50 text-emerald-600 border-emerald-100' : 'bg-amber-50 text-amber-600 border-amber-100'}`}>
          {status}
        </div>
      </div>
      {onAction && !isDone && (
        <button onClick={onAction} className="w-full mt-5 py-3 bg-slate-900 text-white text-[10px] font-bold rounded-xl hover:bg-red-600 transition-colors shadow-lg shadow-slate-200 flex items-center justify-center gap-2 uppercase tracking-tighter">
